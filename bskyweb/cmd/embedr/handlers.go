...
func (srv *Server) getTellusPost(ctx context.Context, did syntax.DID, rkey syntax.RecordKey) (*appbsky.FeedDefs_PostView, error) {
...
func (srv *Server) parseTellusURL(ctx context.Context, raw string) (*syntax.ATURI, error) {
	...
	// then try tellus.app post URL
	u, err := url.Parse(raw)
	if err != nil {
		return nil, err
	}
	if u.Hostname() != "tellus.app" {
		return nil, fmt.Errorf("only tellus.app URLs currently supported")
	}
	...
	if len(pathParts) != 5 || pathParts[1] != "profile" || pathParts[3] != "post" {
		return nil, fmt.Errorf("only tellus.app post URLs currently supported")
	}
...
func (srv *Server) WebOEmbed(c echo.Context) error {
	...
	aturi, err := srv.parseTellusURL(c.Request().Context(), c.QueryParam("url"))
	if err != nil {
		return c.String(http.StatusBadRequest, fmt.Sprintf("Expected 'url' to be tellus.app URL or AT-URI: %v", err))
	}
	...
	post, err := srv.getTellusPost(c.Request().Context(), did, aturi.RecordKey())
	...
	data := OEmbedResponse{
		...
		AuthorURL:    fmt.Sprintf("https://tellus.app/profile/%s", post.Author.Handle),
		ProviderName: "Tellus Social",
		ProviderURL:  "https://tellus.app",
		CacheAge:     86400,
		Width:        &width,
		Height:       nil,
		HTML:         html,
...
func (srv *Server) WebPostEmbed(c echo.Context) error {
	...
	/*
		postView, err := srv.getTellusPost(ctx, did, rkey)
		if err == ErrPostNotFound {
			return c.String(http.StatusNotFound, fmt.Sprintf("%v", err))
		} else if err == ErrPostNotPublic {
			return c.String(http.StatusForbidden, fmt.Sprintf("%v", err))
